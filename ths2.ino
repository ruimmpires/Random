...
#define MQTT_PUBLISH "home/touchless-hands-sanitizer/1/tx"
#define MQTT_RECEIVER "home/touchless-hands-sanitizer/1/rx"
...
int angle = 0;
int IRsensor = 13;  // IR sensor PIN D2
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(),MQTT_USER,MQTT_PASSWORD)) {
      ...
      publishSerialData("home/touchless-hands-sanitizer/1 is online");
      // ... and resubscribe
...
//  delay(1000);
  ...
  if (val == LOW) // When the IR detects a signal
  {
    servo.write(170);               
    //SEND DATA TO MQTT if there is an IR sensed
    //char copy[3];
    //itoa(val,copy,32); 
    publishSerialData(ths);
    delay(1000);
  }
  //Serial.print("val:");
  //Serial.println(val);
